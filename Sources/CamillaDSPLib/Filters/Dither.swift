// CamillaDSP-Swift: Dither filter - noise-shaped dither for bit-depth reduction
// Faithfully matches the Rust CamillaDSP implementation.

import Foundation

// MARK: - Ditherer Protocol & Implementations

/// Protocol matching Rust's `Ditherer` trait. Produces dither in LSB units.
protocol Ditherer {
    init(amplitude: PrcFmt)
    mutating func sample() -> PrcFmt
}

/// No-op ditherer (for `.none` type). Quantization still occurs, just no dither added.
struct NoopDitherer: Ditherer {
    init(amplitude: PrcFmt) {}
    mutating func sample() -> PrcFmt { 0.0 }
}

/// Spectrally-white triangular-pdf (TPDF) dither.
/// Uses a triangular distribution centered at 0 with range [-amplitude/2, amplitude/2].
struct TriangularDitherer: Ditherer {
    ...
    mutating func sample() -> PrcFmt {
        // Triangular distribution on [-halfAmplitude, halfAmplitude] with mode 0
        // Uses inverse CDF method matching Rust's rand_distr::Triangular
        return triangularSample()
    }

    private func triangularSample() -> PrcFmt {
        // Triangular distribution on [-halfAmplitude, halfAmplitude] with mode 0
        let u = PrcFmt.random(in: 0.0...1.0)
        let a = -halfAmplitude
        let b = halfAmplitude
        let c: PrcFmt = 0.0 // mode
        let fc = (c - a) / (b - a) // = 0.5
...
/// Highpass ditherer producing violet noise: current_rpdf - previous_rpdf.
/// This is NOT a noise shaper -- it is a standalone ditherer.
struct HighpassDitherer: Ditherer {
    private var halfAmplitude: PrcFmt
...
/// Noise shaper matching Rust's NoiseShaper: ring buffer + FIR error feedback.
struct NoiseShaper {
    let filter: [PrcFmt]
    /// Ring buffer storing recent errors. Uses push_overwrite semantics:
    /// newest element pushed to end, oldest dropped from front when full.
    ...
    /// Process one sample through the noise shaper.
    /// `scaled` is the input scaled to integer domain (input * scalefact).
    /// `dither` is dither in LSB units.
    /// Returns the rounded result in integer domain.
    mutating func process(scaled: PrcFmt, dither: PrcFmt) -> PrcFmt {
        // Compute error feedback: iterate buffer items paired with reversed coefficients.
        // Rust iterates: buffer.iter().zip(filter.iter().rev())
        // The ring buffer iterator goes from oldest to newest.
        // We need to replicate: iterate the buffer contents (oldest first) zipped with
        // coefficients in reverse order.
        var filtBuf: PrcFmt = 0.0
        let count = filter.count
        for i in 0..<count {
            // buffer contents from oldest to newest
        ...
        // Push error to ring buffer (overwrite oldest)
...
extension NoiseShaper {
    ...
    static func shibataLow48() -> NoiseShaper {
        NoiseShaper(filter: [
            ...
            0.032_142_307_609_319_69, 1.210_869_186_252_239_2e-6, 0.0,
    ...
    static func shibataLow882() -> NoiseShaper {
        NoiseShaper(filter: [
            ...
            0.000_186_019_504_326_395_7, -0.000_534_442_078_787_833_5, -0.000_564_826_827_030_628_9,
            -6.555_314_757_861_197e-5, 0.000_503_513_438_161_462_5, 0.000_697_769_341_059_029_1,
            0.000_215_430_787_648_074_33, 0.000_558_842_846_658_080_8, -0.000_955_912_342_760_711_9,
            -0.000_183_239_637_408_405_54, -0.001_184_734_981_507_063, 5.595_707_625_616_342e-5,
            -0.000_210_925_907_595_083_12, 9.261_416_380_468_29e-6, -1.689_312_557_573_430_2e-5,
            -0.000_102_918_987_977_318_47, -8.705_230_357_008_986e-6, -2.189_383_849_326_986_8e-5,
            2.048_334_863_502_532_2e-5, -9.314_835_915_574_804e-5, -5.457_198_494_696_058e-5,
            1.039_314_793_160_883_7e-5, -4.186_463_047_517_463_6e-5, 3.314_268_178_655_766e-5,
            4.641_250_086_478_976_3e-7, -3.169_075_716_868_974e-5, 2.919_960_388_680_92e-5,
            -4.137_142_968_829_721e-5, 3.097_004_537_266_912e-6, -0.000_130_819_724_290_631_7,
            0.0, 0.0,
        ])
    }
    ...
    static func shibataLow192() -> NoiseShaper {
        NoiseShaper(filter: [
            ...
            -0.004_041_632_637_381_554, -0.000_315_436_191_158_369_2, -6.079_084_869_270_446e-6,
            -2.561_475_230_322_685e-5, -6.444_113_296_311_116e-6, -0.000_143_420_198_583_044_1,
            -9.988_663_229_876_238e-9, -0.000_110_015_651_443_973_18, -0.000_264_444_039_203_226_57,
            -0.018_070_342_019_200_325, -0.013_997_578_062_117_1,
        ])
    }
}
...
/// Dither filter matching CamillaDSP Rust implementation exactly.
///
/// Processing pipeline per sample:
/// 1. Scale input by scalefact (= 2^(bits-1))
/// 2. Generate dither in LSB units
/// 3. If noise shaper: apply error feedback, add dither, round
/// 4. If no shaper: add dither to scaled value, round
/// 5. Descale by dividing by scalefact
public final class DitherFilter: Filter {
    public let name: String
    private var scalefact: PrcFmt

    // Type-erased ditherer (one of Noop, Triangular, Highpass)
    ...
    public init(name: String, config: FilterConfig) {
        ...
        let ditherType = params.ditherType ?? .flat
        self.scalefact = pow(2.0, PrcFmt(bits - 1))

        // Build shaper (if applicable)
        let shaper: NoiseShaper? = Self.makeShaper(for: ditherType)
        ...
        default:
            // All noise-shaped types use default TPDF (amplitude=2.0)
    ...
    public func process(waveform: inout [PrcFmt]) throws {
        for i in 0..<waveform.count {
            ...
            let resultR: PrcFmt
            if var shaper = self.shaper {
                resultR = shaper.process(scaled: scaled, dither: dither)
                self.shaper = shaper
            } else {
