...
struct DevicePickerView: View {
                            ...
                            HStack {
                                ...
                                if appState.resamplerEnabled {
                                    Picker("", selection: $appState.captureSampleRate) {
                                        ForEach(appState.availableCaptureSampleRates, id: \.self) { rate in
                    ...
                    extraContent: AnyView(
                        VStack(alignment: .leading, spacing: 8) {
                            HStack {
                                Text("Sample Rate")
                                    .frame(width: 100, alignment: .leading)
                                Picker("", selection: appState.resamplerEnabled
                                       ? $appState.playbackSampleRate
                                       : $appState.playbackSampleRate) {
                                    ForEach(appState.resamplerEnabled
                                            ? appState.availablePlaybackSampleRates
                                            : appState.commonSampleRates.isEmpty
                                              ? appState.availablePlaybackSampleRates
                                              : appState.commonSampleRates,
                                            id: \.self) { rate in
                                        Text(formatRate(rate)).tag(rate)
                                    }
                                }
    ...
    private var latencyText: String {
        let ms = Double(appState.chunkSize) / Double(appState.captureSampleRate) * 1000.0
        return String(format: "(%.1f ms latency)", ms)
    }
