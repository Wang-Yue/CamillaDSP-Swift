...
struct MiniSpectrumView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            let barWidth = max(2, (geo.size.width - CGFloat(bands.count - 1) * 1.5) / CGFloat(bands.count))
            ...
            VStack {
                ...
                HStack(alignment: .bottom, spacing: 1.5) {
                    ForEach(0..<min(bands.count, 30), id: \.self) { i in
                        let normalized = max(0, min(1, (bands[i] + 60) / 60))
            ...
            GeometryReader { geo in
                ...
                ZStack(alignment: .leading) {
                    ...
                    RoundedRectangle(cornerRadius: 1.5)
                        ...
                        .frame(width: w * normalized(rms), height: h / 2 - 0.5)
                        .offset(y: -(h / 4))
                    ...
                    RoundedRectangle(cornerRadius: 1.5)
                        .fill(
                            LinearGradient(
                                colors: [.green.opacity(0.6), .yellow.opacity(0.6), .orange.opacity(0.6), .red.opacity(0.6)],
                                startPoint: .leading,
                                endPoint: .trailing
                            )
                        )
                        .frame(width: w * normalized(peak), height: h / 2 - 0.5)
                        .offset(y: h / 4)
                }
            }
